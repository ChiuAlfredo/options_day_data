...
pipeline = [
    {
        ...
        "$match": {
            ...
            "$expr": {
                "$and": [
                    {"$eq": [{"$hour": "$時間"}, 12]},  # 时间等于 12:00
                    {"$gte": [{"$minute": "$時間"}, 30]},  # 分钟数大于等于 30
                    {"$lte": [{"$minute": "$時間"}, 55]}  # 分钟数小于等于 55
                ]
            }
        }
    },
...
end_date_hl.to_csv('9點20天平均.csv', encoding='utf-8-sig', index=False)
...
},
{
    "$project": {
        ...
        "lowestPrice": 1,
        "high_low_diff": {"$subtract": ["$highestPrice", "$lowestPrice"]},
        "high_low_diff9-10":{"$subtract": ["$highest_in_9_to_10", "$lowest_in_9_to_10"]},
        "high_low_diff9-12":{"$subtract": ["$highest_in_9_to_12", "$lowest_in_9_to_12"]},
        "highestPriceTime": {
            "$arrayElemAt": [
                {
                    "$filter": {
        ...
        "highest_in_9_to_12": 1,
        "lowest_in_9_to_12": 1,
        "highest_in_9_to_12_time": {
            "$arrayElemAt": [
                {
                    "$filter": {
                        "input": "$records",
                        ...
                        "cond": {
                            "$and": [
                                {"$eq": ["$$item.最高價", "$highest_in_9_to_12"]},
                                {"$gte": [{"$hour": "$$item.時間"}, 9]},
                                {"$lt": [{"$hour": "$$item.時間"}, 12]}
                            ]
                        }
...
},
{
    "$project": {
        ...
        "highest_in_9_to_12": 1,
        "highest_in_9_to_12_time": "$highest_in_9_to_12_time.時間",
        "lowest_in_9_to_10": 1,
        "lowest_in_9_to_10_time": "$lowest_in_9_to_10_time.時間",
        "lowest_in_9_to_12": 1,
        "lowest_in_9_to_12_time": "$lowest_in_9_to_12_time.時間"
    }
},
{
    "$sort": {"成交日期": 1}
...
result_df['20天平均'] = result_df['high_low_diff'].rolling(window=20).mean()
result_df['20天平均_90-10'] = result_df['high_low_diff9-10'].rolling(window=20).mean()
result_df['20天平均_90-12'] = result_df['high_low_diff9-12'].rolling(window=20).mean()
...
end_date_hl['highestPriceTime_cal'] = [time_to_minutes(t) for t in end_date_hl['highestPriceTime']]
end_date_hl['lowestPriceTime_cal'] = [time_to_minutes(t) for t in end_date_hl['lowestPriceTime']]
end_date_hl['highestPriceTime_cal_9_to_10'] = [time_to_minutes(t) for t in end_date_hl['highest_in_9_to_10_time']]
end_date_hl['lowestPriceTime_cal_9_to_10'] = [time_to_minutes(t) for t in end_date_hl['lowest_in_9_to_10_time']]
end_date_hl['highestPriceTime_cal_9_to_12'] = [time_to_minutes(t) for t in end_date_hl['highest_in_9_to_12_time']]
end_date_hl['lowestPriceTime_cal_9_to_12'] = [time_to_minutes(t) for t in end_date_hl['lowest_in_9_to_12_time']]
...
pipeline = [
    ...
    "$group": {
        ...
        "highestPrice": {"$max": "$最高價"},
        "lowestPrice": {"$min": "$最低價"},
        "highest_in_12_30_to_12_55": {
            "$max": {
                "$cond": {
                    "if": {
                        "$and": [
                            {"$eq": [{"$hour": "$時間"}, 12]},  # 小时等于 12
                            {"$gte": [{"$minute": "$時間"}, 30]},  # 分钟大于等于 30
                            {"$lte": [{"$minute": "$時間"}, 55]}  # 分钟小于等于 55
                        ]
                    },
                    "then": "$最高價",
                    "else": None
...
},
{
    "$project": {
        ...
        "lowestPrice": 1,
        "high_low_diff": {"$subtract": ["$highestPrice", "$lowestPrice"]},
        "high_low_diff1230-1255": {"$subtract": ["$highest_in_12_30_to_12_55", "$lowest_in_12_30_to_12_55"]},
        "highestPriceTime": {
            "$arrayElemAt": [
                {
                    "$filter": {
        ...
        "highest_in_12_30_to_12_55": 1,
        "highest_in_12_30_to_12_55_time": {
            "$arrayElemAt": [
                {
                    "$filter": {
                        "input": "$records",
                        ...
                        "cond": {
                            "$and": [
                                {"$eq": ["$$item.最高價", "$highest_in_12_30_to_12_55"]},
                                {"$eq": [{"$hour": "$$item.時間"}, 12]},  # 小时等于 12
                                {"$gte": [{"$minute": "$$item.時間"}, 30]},  # 分钟大于等于 30
                                {"$lte": [{"$minute": "$$item.時間"}, 55]}  # 分钟小于等于 55
                            ]
                        }
                    }
                },
        ...
        "lowest_in_12_30_to_12_55": 1,
        "lowest_in_12_30_to_12_55_time": {
            "$arrayElemAt": [
                {
                    "$filter": {
                        "input": "$records",
                        ...
                        "cond": {
                            "$and": [
                                {"$eq": ["$$item.最低價", "$lowest_in_12_30_to_12_55"]},
                                {"$eq": [{"$hour": "$$item.時間"}, 12]},  # 小时等于 12
                                {"$gte": [{"$minute": "$$item.時間"}, 30]},  # 分钟大于等于 30
                                {"$lte": [{"$minute": "$$item.時間"}, 55]}  # 分钟小于等于 55
                            ]
...
},
{
    "$project": {
        ...
        "lowestPrice": 1,
        "high_low_diff": 1,
        "high_low_diff1230-1255": 1,
        "highestPriceTime": "$highestPriceTime.時間",
        "lowestPriceTime": "$lowestPriceTime.時間",
        "highest_in_12_30_to_12_55": 1,
        "highest_in_12_30_to_12_55_time": "$highest_in_12_30_to_12_55_time.時間",
        "lowest_in_12_30_to_12_55": 1,
        "lowest_in_12_30_to_12_55_time": "$lowest_in_12_30_to_12_55_time.時間"
    }
},
{
    "$sort": {"成交日期": 1}
...
pipeline = [
    ...
    {
        "$group": {
            "_id": {"$dateToString": {"format": "%Y-%m-%d", "date": "$分鐘時間"}},
            "highestPrice": {"$max": "$最高價"},
            "lowestPrice": {"$min": "$最低價"},
            "records": { "$push": "$$ROOT" }  # 保存所有原始記錄
        }
                ...
                "$and": [
                    {
                        "$or": [
                            {
                                "$and": [
                                    {"$eq": [{"$hour": "$分鐘時間"}, 12]},  # 小时等于 12
                                    {"$gte": [{"$minute": "$分鐘時間"}, 30]}  # 分钟大于等于 30
                                ]
                            },
                            {"$eq": [{"$hour": "$分鐘時間"}, 13]},  # 小时等于 13
                            {
                                "$and": [
                                    {"$eq": [{"$hour": "$分鐘時間"}, 13]},  # 小时等于 14
                                    {"$lte": [{"$minute": "$分鐘時間"}, 30]}  # 分钟小于等于 30
                                ]
                            }
                        ]
                    }
            ...
            "$arrayElemAt": [
                {
                    "$filter": {
                        ...
                        "cond": {
                            "$and": [
                                {"$eq": ["$$item.最高價", "$highest_in_12_30_to_12_55"]},
                                {"$eq": [{"$hour": "$$item.分鐘時間"}, 12]},  # 小时等于 12
                                {"$gte": [{"$minute": "$$item.分鐘時間"}, 30]},  # 分钟大于等于 30
                                {"$lte": [{"$minute": "$$item.分鐘時間"}, 55]}  # 分钟小于等于 55
                            ]
                        }
                    }
                },
...
},
{
    "$project": {
        ...
        "high_low_diff": 1,
        "high_low_diff1230-1255": 1,
        "highestPriceTime": "$highestPriceTime.分鐘時間",
        "lowestPriceTime": "$lowestPriceTime.分鐘時間",
        "highest_in_12_30_to_12_55": 1,
        "highest_in_12_30_to_12_55_time": "$highest_in_12_30_to_12_55_time.分鐘時間",
        "lowest_in_12_30_to_12_55": 1,
        "lowest_in_12_30_to_12_55_time": "$lowest_in_12_30_to_12_55_time.分鐘時間"
    }
},
{
    "$sort": {"成交日期": 1}
...
end_date_hl.to_csv('12點20天平均_index.csv', encoding='utf-8-sig', index=False)