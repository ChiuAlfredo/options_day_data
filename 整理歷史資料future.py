...
def to_minutes_future(data_list):
    ...
    grouped_df = df.groupby(['成交日期', '商品代號', '到期月份(週別)','時間'])
    result = grouped_df.agg(
        開盤價=('成交價格', 'first'),
        收盤價=('成交價格', 'last'),
        最高價=('成交價格', 'max'),
        最低價=('成交價格', 'min'),
        成交量=('成交數量(B+S)', lambda x: x.astype(int).sum() / 2)
            ).reset_index()
...
def read_rpt_insert_future(rpt_path):
    ...
    result_list = to_minutes_future(data_list)

    # 將數據插入到 MongoDB
    price_collection.insert_many(result_list)
    print(f'{rpt_path} has been inserted into MongoDB')
...
for folder in os.listdir(folder_path)[-3:]:
    ...
    if os.path.isdir(os.path.join(folder_path, folder)):
        for file in os.listdir(os.path.join(folder_path, folder)):
            # file = os.listdir(os.path.join(folder_path, folder))[15]
            if file.endswith('.rpt'):
                rpt_path = os.path.join(folder_path, folder, file)
                read_rpt_insert_future(rpt_path)
            elif file.endswith('.zip'):
                try:
                    zip_path = os.path.join(folder_path, folder, file)
                    unzip_file(zip_path, os.path.join(folder_path, folder))
                    rpt_file = file[:-4] + '.rpt'
                    rpt_path = os.path.join(folder_path, folder, rpt_file)
                    read_rpt_insert_future(rpt_path)
