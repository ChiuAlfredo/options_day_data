...
hl_20_df = pd.read_csv('20天平均.csv')
    ...
    for future_index, future_row in month_future_price_df_raw.iterrows():
        ...
        if (future_row['時間'].strftime("%Y-%m-%d") == end_date.strftime("%Y-%m-%d")) & (future_row['時間'].hour >= 9):
            ...
            if (future_row['時間'].hour == 10) & (future_row['時間'].minute >1 ) & (option.position_list['option']==[])& (n>0):
                ...
                hl_20_row = hl_20_df[hl_20_df['結算日']==future_row['時間'].strftime("%Y-%m-%d")]
                if (high_9-low_9)<100 or (high_9-low_9)<hl_20_row['20天平均'].values[0]:

                
                    call_strike = find_call_near(low_9+hl_20_row['20天平均'].values[0]*average)
                    put_strike = find_put_near(high_9-hl_20_row['20天平均'].values[0]*average)
...
for average in [0.5,0.6,0.7,0.8,0.9,1]:
    ...
    with pd.ExcelWriter(f'record_list_9hl_average{stop_loss}_{average}_{hedge_frequency}_{record_list[0]["日期"]}-{record_list[-1]["日期"]}.xlsx') as writer:
