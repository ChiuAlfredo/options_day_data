...
df_info = pd.DataFrame(columns=['month','underlayed', 'kind' , 'strike_price','MTF_PROD_ID', 'previous_last_price','last_first_price', 'last_last_price','previous_day','last_day'])
...
for file_folder in file_folders:
    ...
    for strike_price in month_data_df['strike_price'].unique():
        ...
        # 計算最後一天變動%數
        previous_last_trade,last_first_trade,last_last_trade,last_day,previous_day = count_util.count_last_day_change(df_strike_price)
...
# 讀取TX2-4.csv期貨價格
TX_df = pd.read_csv('TX2-4-1.csv',encoding='utf-8',sep='\t')
...
def get_data(date,df,column):
    ...
    if not prices.empty:
        ...
        return None  # Or any other value you want to use when there is no match

# 前一天期貨收盤價
df_info['previous_day_index_price'] = df_info.apply(lambda row: get_data(row['previous_day'],TX_df,column ='收盤價(元)'), axis=1)

# 前一天期貨收盤價
df_info['last_day_index_price'] = df_info.apply(lambda row: get_data(row['last_day'],TX_df,column ='收盤價(元)'), axis=1)

# 計算previous_last_first變動%
df_info['previous_last_day_change'] = (df_info['last_last_price'] - df_info['previous_last_price']) / df_info['previous_last_price'] * 100
...
# 計算價內價外
df_info['previous_ITM_OTM'] = df_info.apply(
    lambda row: 'ITM' if pd.notnull(row['previous_day_index_price']) and row['previous_day_index_price'] > row['strike_price'] 
                else ('OTM' if pd.notnull(row['previous_day_index_price']) else None), 
    axis=1)


# 新增sigma
vitx_df = pd.read_csv('vitx.csv',encoding='utf-16',sep='\t')
vitx_df['datetime'] = pd.to_datetime(vitx_df['年月日'], format='%Y%m%d')
vitx_df.set_index('datetime', inplace=True)

# 前一天sigma
df_info['previous_day_sigma'] = df_info.apply(lambda row: get_data(row['previous_day'],vitx_df,column ='收盤波動率指數'), axis=1)
# 轉成小數點
...
df_info['previous_bs_price'] = df_info.apply(
    lambda row: BS_CALL(row['previous_day_index_price'], row['strike_price'], t, r, row['previous_day_sigma']) 
    if row['kind'] == 'C' 
    else BS_PUT(row['previous_day_index_price'], row['strike_price'], t, r, row['previous_day_sigma']), 
    axis=1)


# 排序
df_info.sort_values(by=['month','underlayed', 'kind', 'strike_price'], inplace=True)

# 周選
df_info[df_info['underlayed'].str.contains('TX1|TX2|TX4|TX5')].to_csv('week_options_last_day_info.csv', index=False, encoding='utf-8-sig')

# 月選
df_info[df_info['underlayed'].str.contains('TXO')].to_csv('month_options_last_day_info.csv', index=False, encoding='utf-8-sig')
...
def draw_prop_change(df_info,df_title):
    ...
    draw_util.draw_prop_change(df_info = df_info,column='previous_last_day_change',title = f'{df_title}_prob_previous_last')
    draw_util.draw_prop_change(df_info = df_info[df_info['previous_ITM_OTM']=='OTM'],column='previous_last_day_change',title = f'{df_title}_prob_previous_last_OTM')
    draw_util.draw_prop_change(df_info = df_info[df_info['previous_ITM_OTM']=='ITM'],column='previous_last_day_change',title = f'{df_title}_prob_previous_last_ITM')
    draw_util.draw_prop_change(df_info = df_info[df_info['previous_last_price']>df_info['previous_bs_price']],column='previous_last_day_change',title = f'{df_title}_prob_previous_last_price>bs')
    draw_util.draw_prop_change(df_info = df_info[df_info['previous_last_price']<df_info['previous_bs_price']],column='previous_last_day_change',title = f'{df_title}_prob_previous_last_price<bs')

# 月選
month_options_df = pd.read_csv('month_options_last_day_info.csv',encoding='utf-8')

draw_prop_change(df_info = month_options_df,df_title = 'month')

# 周選
week_options_df = pd.read_csv('week_options_last_day_info.csv',encoding='utf-8')
