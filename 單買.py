...
def process_row(index, row, end_date_df, result_queue):
    ...
    for future_index, future_row in month_future_price_df_raw.iterrows():
        ...
        if (future_row['時間'].strftime("%Y-%m-%d") == end_date.strftime("%Y-%m-%d")) & (future_row['時間'].hour >= 9):
            ...
            if (future_row['時間'].hour == 9) & (future_row['時間'].minute >0 ) & (option.position_list['option']==[])& (n>0):
                ...
                option_row_cal =  filter_merged_df(merged_df, 時間=future_row['時間'], 履約價格=choose_strike, 買賣權別='C')
                ...
                option.option_trade(future_row['時間'],'B','C',choose_strike,option_row_cal['選擇權_收盤價'].values[0],1,0,position_action='open',record_dict={'iv':option_row_cal['iv'].values[0]})
    ...
    if n==0:
        record_dict = {
            ...
            'call_價格': merged_df[(merged_df['履約價格'] == choose_strike) & (merged_df['買賣權別'] == 'C')][['時間', '選擇權_收盤價', 'iv']],
